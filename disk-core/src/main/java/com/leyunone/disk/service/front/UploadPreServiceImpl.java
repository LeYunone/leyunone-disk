...
public class UploadPreServiceImpl implements UploadPreService {
     ...
     */
    @Override
    public FileValidatorVO judgeFile(MultipartFile multipartFile) {
        FileValidatorVO result = new FileValidatorVO();
        AssertUtil.isFalse(ObjectUtil.isNull(multipartFile), "file is empty");
        ...
        try {
            ...
            if (ObjectUtil.isNotNull(fileMd5DO)) {
                ...
                //返回给前端：不用继续操作
                resultType = ObjectUtil.isNull(fileInfoDO) ? 1 : 0;
            } else {
